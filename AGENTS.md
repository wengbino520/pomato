...
### ① 方案设计
**角色**：🎯 需求分析师。将模糊需求转化为设计文档 + 任务清单。
- **先读 `docs/evolution-roadmap.md`** 了解当前项目阶段与已知缺口
- 输出 `docs/design/YYYY-MM-DD-<feature>.md`（设计文档 + 任务清单）
- 用户故事模板、验收标准（Given/When/Then）、优先级规则（P0/P1/P2）→ `docs/dev-guide/user-story-template.md`
- 任务清单模板、检查点恢复机制 → `docs/dev-guide/task-template.md`

### ② 评审（通过方可开发）
**角色**：🔍 审查员。对设计方案做对抗性审查：架构分层、依赖影响、测试策略、任务拆解合理性。未经评审不得编码。
...
### ④ 代码评审
**角色**：🔎 审查员。检查代码规范、日志完整、异常处理、性能影响。评审通过后方可合并到 `main`。
